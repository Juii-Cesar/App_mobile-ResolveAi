...
export default function TelaPrincipalProfissional({ navigation }) {
    ...
    (async () => {
      ...
      if (status !== 'granted') {
        setModalAlerta({
          ...
          mensagem: 'Precisamos da sua localização para o aplicativo funcionar e receber pedidos próximos.',
          tipo: 'aviso'
        });
        return;
      }
    ...
    useCallback(() => {
      async function carregarDadosIniciais() {
        try {
          ...
          const { data: dataProfissoes } = await supabase.from('profissoes_profissional').select('profissao').eq('profissional_id', user.id);
          ...
          if (!servicoAtivo) {
            const { data: dataAtivo } = await supabase.from('servicos').select('id, descricao, cliente:usuarios!idcliente(nome)').eq('idprofissional', user.id).eq('status', 'servicoAceito').maybeSingle();
            if (dataAtivo) {
              iniciarServico({
                id: dataAtivo.id,
  ...
  useEffect(() => {
    ...
    async function escutarServicos() {
      ...
      const { data: pendentes } = await supabase
        .from('servicos')
        .select('id, descricao, latitude, longitude, endereco, cliente:usuarios!idcliente(nome)')
  ...
  function handleIniciar() {
    if (!locationRef.current) {
      setModalAlerta({
        ...
        mensagem: 'Ainda estamos buscando a sua localização via GPS. Tente novamente em alguns segundos.',
        tipo: 'aviso'
      });
      return;
    }
    if (!profissaoSelecionada && profissoes.length > 0) {
      setModalAlerta({
        titulo: 'Atenção',
        mensagem: 'Selecione como deseja atuar antes de ficar online!',
        tipo: 'aviso'
      });
      return;
    }
  ...
  async function handleAceitar() {
    ...
    if (!error) {
      ...
      setModalAlerta({
        titulo: 'Poxa!',
        mensagem: 'Este serviço não está mais disponível. Outro profissional pode ter aceitado antes.',
        tipo: 'erro'
      });
      handleRecusar();
    }
    ...
    try {
      ...
      if (resultados.length > 0) {
        ...
        setModalAlerta({
          ...
          mensagem: 'Não conseguimos encontrar esse local. Tente digitar o nome da cidade junto.',
          tipo: 'aviso'
        });
      }
    } catch (error) {
      setModalAlerta({
        ...
        mensagem: 'Ocorreu um problema ao buscar o endereço. Verifique sua conexão.',
        tipo: 'erro'
      });
    }
  }
      ...
      <Modal animationType="fade" transparent={true} visible={!!modalAlerta} onRequestClose={handleFecharAlerta}>
        <View style={styles.modalFundoOverlay}>
          <View style={styles.modalCard}>
            <Ionicons 
              name={modalAlerta?.tipo === 'aviso' ? 'warning' : 'alert-circle'} 
              size={80} 
              color={modalAlerta?.tipo === 'aviso' ? '#F5A623' : '#D32F2F'} 
              style={{ marginBottom: 10 }} 
            />
            <Text style={[styles.modalTitle, { color: modalAlerta?.tipo === 'aviso' ? '#F5A623' : '#D32F2F' }]}>
              {modalAlerta?.titulo}
            </Text>
            <Text style={styles.modalText}>{modalAlerta?.mensagem}</Text>
            
            <TouchableOpacity 
              style={[styles.modalBtnEntendi, { backgroundColor: modalAlerta?.tipo === 'aviso' ? '#F5A623' : '#D32F2F' }]} 
              onPress={handleFecharAlerta} 
              activeOpacity={0.8}
            >
        ...
        <View style={[styles.headerFlutuante, { justifyContent: servicoAtivo ? 'space-between' : 'flex-end' }]} pointerEvents="box-none">
          ...
          {servicoAtivo && (
            <TouchableOpacity 
              ...
              onPress={() => navigation.navigate('TelaChatProfissional', { profissionalNome: servicoAtivo.nomeCliente, dadosServico: servicoAtivo })}
            >
